...
The answer is that there are actually two variants of the `update` function: For apps that are able to respond immediately to UI as shown above you can create the simple version of the `update` function demonstrated which returns just a new model. For apps whose activity may delay the response from the `update` functions they can return both the new model and one or more command functions encapsulating that activity as follows:
...
We have encountered `Dispatch<'msg>` previously. It is the type of the `dispatch` argument to the normal MVU `view` function. It is simply an alias for a function that accepts a message and sends it immediately to the MVU update loop so that it ends up in `update`.
...
The final alias, `Cmd<'msg>`, is just a list of `Sub<'msg>`, i.e. a list of `Dispatch<'msg> -> unit` functions. In other words, the `update` function can return a list of `Dispatch<'msg> -> unit` functions that the MVU update loop will execute by passing them a dispatch function. These functions, as you saw above, can then dispatch any message at any time. Therefore, if you need to do impure stuff such as calling a web API, you simply create a function accepting `dispatch`, perform the call there (likely using `async`), and then use the `dispatch` argument to dispatch a message when you receive a response.

In other words, unlike the model which is next given to the view function for interpretation the `Cmd<'msg>` function will be by the MVU. From the point of view of your model, everything happens asynchronously: your app and UI update loop continues without waiting for the results of the `Cmds<'msg>` and when the commands complete they the results are `dispatch` into the event loop as just another event.

For example:

* The user clicks a button to log in, which dispatches a `SignInRequested` message
* The `update` function returns a new `model` with an `IsBusy = true` value (which can be shown as an animation such as a spinner) as well as a `Cmd<'msg>` function that will call the API to perform the request and process the response asynchronously onced invoked with a `dispatch` function.
* MVU uses model with view to update the UI and invokes the command by passing it the `dispatch` function.
...
* When the API call returns (as detected by `await` completes or `Task` completion event), the function that called the API continues and by calling the `dispatch` function inserts the result into the MVU eventloop (e.g. `SignInSuccessful` or `SignInFailed`) for the `update` function to immediately handle it.
